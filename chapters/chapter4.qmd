...
Esta integración cierra el ciclo del **Sistema Integrado de Decisión**, vinculando la micro-escala de la navegación aérea segura con la macro-escala de la logística territorial eficiente. La @fig-flujo-decision ilustra el flujo completo del sistema de decisión desde la adquisición de datos hasta la recomendación operacional.
...
### Sensibilidad frente a condiciones meteorológicas adversas

La robustez del sistema integrado se evalúa mediante un análisis de sensibilidad frente a la degradación de las condiciones meteorológicas. Sea $\theta \in [0,1]$ un parámetro de severidad meteorológica, donde $\theta = 0$ representa condiciones óptimas y $\theta = 1$ condiciones extremas. La probabilidad de vuelo operativo se modela como:

$$
p_{\text{vuelo}}(\theta) = p_0 \cdot (1 - \theta)^\kappa,
$$

con $p_0 = 0.94$ (probabilidad base) y $\kappa = 2.5$ (factor de sensibilidad empírico).

La @fig-sensibilidad-meteo ilustra la relación entre severidad meteorológica y eficiencia logística normalizada. Se observa que el sistema mantiene eficiencia superior al 80% para $\theta < 0.4$, degradándose gradualmente para condiciones más adversas. Esta característica es deseable, ya que refleja una transición suave entre modos operativos en lugar de fallos catastróficos.
...
#### 1. Ventana temporal óptima para evaluación
...
#### 2. Umbrales de decisión conservadores
...
#### 3. Protocolos de contingencia escalonados
...
#### 4. Integración con sistemas de monitoreo existentes
...
#### 1. Dependencia de densidad de observaciones
...
#### 2. Suposición de advección unidimensional
...
#### 3. Estática de los umbrales técnicos
...
#### 4. Independencia condicional entre variables
...
#### 1. Optimización multiobjetivo dinámica
...
#### 2. Aprendizaje automático híbrido
...
#### 4. Coordinación de enjambres de drones
...
#### 5. Asimilación de datos satelitales
...
#### 6. Extensión geográfica a otras regiones
