...
2. **Construcción de la réplica**: Los bloques seleccionados se concatenan secuencialmente para formar la serie temporal bootstrap:
   $$
   \{y_i^{(b)}(t)\}_{t=1}^{KL/\Delta t} = \bigoplus_{m=1}^K \mathcal{B}_{k_m^{(b)}}
   $$
...
La implementación computacional del método se detalla en el Apéndice A, donde se presentan las funciones principales del algoritmo. El código completo está disponible en el repositorio
