...
## Procesos estocásticos de segundo orden y espacio de Hilbert $L^2(\Omega,\mathcal{F},\mathbb{P})$
...
## Correlación cruzada y existencia del lag óptimo discreto
...
Bajo la hipótesis nula de independencia entre $\{X_t\}$ y $\{Y_t\}$, Bartlett demostró que para $|k| \ll T$ la distribución muestral es asintóticamente normal con varianza aproximada $1/T$ [@bartlett1946theoretical]. Más precisamente, si $\{X_t\}$ y $\{Y_t\}$ son procesos gaussianos independientes:
...
donde $\Delta s$ es la distancia geodésica entre centroides y $\Delta t$ el intervalo de muestreo temporal. Para evitar estimaciones espurias en presencia de baja correlación, se introduce un criterio de aceptación basado en la fracción de pares adyacentes que superan $\rho_{\min}$ simultáneamente con $k^* \neq 0$, garantizando robustez frente a ruido de medición y fluctuaciones no advectivas [@brockwell2016time].
...
$$
Aunque el periodograma es asintóticamente insesgado ($\mathbb{E}[I_T(\omega)] \to f(\omega)$ cuando $T \to \infty$), no es consistente: su varianza no converge a cero, incluso para $T$ grande. Para obtener estimadores consistentes se requiere suavizado espectral, ya sea mediante ventanas espectrales (Daniell, Parzen, Bartlett) o mediante promediado sobre frecuencias adyacentes [@priestley1981spectral].

Alternativamente, los métodos paramétricos ajustan un modelo ARMA de orden finito a la serie y derivan $f(\omega)$ analíticamente a partir de los coeficientes estimados. Estos enfoques son particularmente útiles cuando la longitud muestral es limitada, como ocurre en ventanas de nowcasting de 7 días con muestreo cada 10 minutos.
...
Este objeto matemático cuantifica la energía asociada a cada par $(\omega, \kappa)$ y permite identificar modos de propagación dominantes como regiones de alta concentración espectral en el plano $f$-$k$ [@brillinger1981time].
...
Esta relación lineal entre fase y posición espacial constituye el fundamento del método de **pendiente de fase** para estimar la velocidad de propagación. Si $\phi(s_i, \omega)$ se desenvuelve adecuadamente (*phase unwrapping*), el número de onda efectivo a frecuencia $\omega$ se obtiene mediante regresión lineal:
...
2. **Ponderación espectral**: Las frecuencias con baja energía presentan fases dominadas por ruido de medición, produciendo estimaciones de $\hat{\kappa}$ inestables. Se introduce un peso proporcional a la densidad espectral:
   ...
   $$
   y la velocidad final se calcula como la mediana ponderada de $\{v_p(\omega_m)\}$ sobre una banda de interés fisico-meteorológico [@bendat2010random].
...
El análisis $f$-$k$ complementa la estimación por correlación cruzada (Sección 2.3) en regímenes donde la coherencia temporal entre segmentos adyacentes es baja pero existe estructura espectral clara. La velocidad estimada $v = \operatorname{median}_{\omega}\{v_p(\omega)\}$ se incorpora directamente al operador de transporte advectivo discreto:
$$
s_i^{\text{up}} = s_i - v H,
$$

que constituye el núcleo del modelo híbrido de nowcasting desarrollado en la Sección 4.4. Esta cadena teórica, desde la transformada 2D hasta la regresión de fase, garantiza que el pronóstico respete la física de propagación observada, transformando observaciones espacio-temporales en parámetros operativos cuantificables y trazables.

## Modelos autorregresivos AR(1) y persistencia temporal {#sec-ar1}

Los modelos autorregresivos de orden uno constituyen la clase más simple y ampliamente utilizada de procesos estocásticos lineales. Su estructura recursiva captura la inercia temporal inherente a variables meteorológicas como la temperatura y la humedad relativa, permitiendo representar la persistencia observada en escalas de minutos a horas sin requerir parámetros adicionales. En el contexto de nowcasting, el modelo AR(1) proporciona el componente local del pronóstico híbrido descrito en la Sección 4.4, complementando el transporte advectivo con dinámica temporal intrínseca y garantizando estabilidad numérica frente a fluctuaciones de alta frecuencia [@brockwell2016time].
...
$$
la cual converge en media cuadrática ($L^2$) dado que $\sum_{j=0}^\infty |\phi|^j = (1-|\phi|)^{-1} < \infty$ [@brockwell2016time].
...
La estructura exponencial de la autocovarianza implica que la dependencia temporal decae monótonamente con el desfase $h$. El parámetro $\phi$ cuantifica directamente la **persistencia temporal**: valores cercanos a $1$ indican alta inercia (variabilidad lenta), mientras que $\phi \approx 0$ corresponde a comportamiento cercano al ruido blanco [@shumway2017time].
...
En la implementación del sistema de nowcasting (Sección 4.4), esta expresión se utiliza directamente para calcular el componente autorregresivo:
$$
...
$$
Bajo la hipótesis de estacionariedad y ruido blanco, $\hat{\phi}$ es consistente y asintóticamente normal:
$$
\sqrt{T}(\hat{\phi} - \phi) \xrightarrow{d} \mathcal{N}\left(0, 1-\phi^2\right), \quad T \to \infty.
$$
Para series meteorológicas de longitud limitada, la varianza de $\hat{\phi}$ se infla cuando $|\phi|$ se aproxima a $1$ (raíz unitaria cercana). Por esta razón, en segmentos con escasez de observaciones se aplica regularización mediante truncamiento o sustitución por la mediana espacial de coeficientes válidos, garantizando estabilidad operativa [@box2015time].
...
El método bootstrap clásico, propuesto por Efron, constituye una herramienta poderosa para aproximar distribuciones muestrales bajo la hipótesis de independencia e idéntica distribución (i.i.d.) [@efron1979bootstrap]. Sin embargo, en el análisis de series meteorológicas y en la cuantificación de incertidumbre de pronósticos de corto horizonte, esta suposición es estructuralmente inválida: las observaciones presentan dependencia serial, ciclos diurnos y memoria termodinámica que deben preservarse en cualquier procedimiento de remuestreo. El **bootstrap de bloques** (*block bootstrap*) surge como la extensión natural que mantiene la estructura de correlación temporal al remuestrear segmentos contiguos en lugar de observaciones aisladas [@kunsch1989jackknife].

### Formulación matemática del Moving Block Bootstrap (MBB)

Sea $\{X_t\}_{t=1}^T$ una trayectoria observada de un proceso estacionario débilmente dependiente. Para una longitud de bloque fija $L \in \mathbb{N}$ con $L \ll T$, se definen los bloques superpuestos:
$$
