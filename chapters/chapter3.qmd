...
format:
  html: 
    ...
    crossrefs:
      sections: true 
    css: 
      - custom.css  # Archivo CSS externo  
  pdf: default
execute: 
  freeze: auto 
jupyter: python3
...
Para abordar este problema de optimización logística, se propone la aplicación del algoritmo PageRank a la red territorial del estado (@sec-pagerank). Originalmente desarrollado por @brin1998anatomy para el ordenamiento de páginas web, este algoritmo fundamenta su operación en la teoría de cadenas de Markov de tiempo discreto (@sec-markov), proporcionando una medida de centralidad que identifica nodos estratégicos dentro de una red compleja.

Matemáticamente, sea $G = (V, E, W)$ un grafo dirigido ponderado que representa la red territorial (@sec-grafos), donde $V$ denota el conjunto de vértices (municipios), $E \subseteq V \times V$ el conjunto de aristas (conexiones viales), y $W: E \to \mathbb{R}^+$ una función que asigna a cada arista $(v_i, v_j)$ la distancia tridimensional $d_{ij}^{(3D)}$ entre los municipios $i$ y $j$. El algoritmo PageRank calcula el vector estacionario $\pi \in \mathbb{R}^{124}$ que satisface:

$$
\pi = P\pi, \quad \sum_{i=1}^{124} \pi_i = 1, \quad \pi_i > 0 \; \forall i,
$$

donde $P$ es la matriz de transición (@sec-transicion) modificada que incorpora un factor de amortiguamiento $p = 0.85$ para garantizar la ergodicidad de la cadena (@sec-ergodicidad). El valor $\pi_i$ representa la importancia relativa del municipio $i$ dentro de la red, permitiendo identificar aquellos nodos que actúan como centros de distribución naturales.
...
5. Identificación de rutas óptimas desde el punto de almacenamiento (Protección Civil en Tuxtla Gutiérrez) hacia los municipios prioritarios. 
...
::: {.columns style="display: flex; gap: 25px;"}
::: {.column width="46%"}

![](puente.png){style="height:250px; width:auto; object-fit:contain;"}
<small><em>Figura 1a. Reconstrucción del puente sobre el Río Novillero, autopista Mapastepec–Tapachula. Participación de CMIC y SICT (2025).</em></small> 

:::

::: {.column width="46%"}

![](caida_puente.png){style="height:250px; width:auto; object-fit:contain;"}
<small><em>Figura 1b. Colapso del puente Tablazón I en Ulapa, Mapastepec, tras intensas lluvias (2025). Fuente: Protección Civil Chiapas.</em></small>
...
### Procedimiento de integración de datos {#sec-nominatim2}
...
Como ya se comentó el estudio consideró la totalidad de los 124 municipios que conforman el estado de Chiapas ([lista completa de municipios](https://github.com/AngelaOrtiz25/tesis_angela/blob/gh-pages/datos_codigos/lista_de_municipios_Chiapas.csv))

### Implementación computacional

El [código completo](https://github.com/AngelaOrtiz25/tesis_angela/blob/gh-pages/datos_codigos/recolecion_de_datos_pagerank.ipynb) para la extracción y procesamiento de los datos garantiza la transparencia y reproducibilidad del estudio. Las funciones principales implementadas incluyen:

- `cargar_mosaico_srtm()`: Carga y fusiona múltiples teselas DEM en un único mosaico ráster.
- `obtener_coordenadas(ciudad)`: Obtiene las coordenadas geográficas de un municipio mediante Nominatim ver @sec-nominatim2, inciso 2
...
## Cálculo de distancias geodésicas 2D y 3D {#sec-distancias3d}
### Formulación matemática de la distancia geodésica bidimensional

Con el objetivo de estimar la longitud real de las rutas considerando la curvatura terrestre, se empleó la fórmula de Vincenty (@vincenty1975) sobre el elipsoide WGS84 (*World Geodetic System 1984*). Sea una ruta discretizada por $N$ puntos geográficos con coordenadas:
...
Esta expresión corresponde a la norma euclidiana en $\mathbb{R}^3$ del vector desplazamiento entre puntos consecutivos, proyectado sobre un sistema de coordenadas locales tangente a la superficie terrestre. La aproximación es válida cuando $\lvert \Delta h_i < d_i^{(2D)}\rvert$, condición satisfecha en la mayoría de las rutas viales analizadas en el estado de Chiapas.
...
El cálculo de distancias se implementó utilizando la librería `pyproj` (@pyproj_library) una interfaz de Python para la biblioteca de proyecciones cartográficas [PROJ](https://pyproj4.github.io/pyproj/stable/). Esta librería proporciona  proporciona funciones optimizadas para geodesia elipsoidal. La implementación garantiza consistencia numérica y reproducibilidad, aspectos fundamentales para la validación científica de los resultados.

El [código completo](https://github.com/AngelaOrtiz25/tesis_angela/blob/gh-pages/datos_codigos/recolecion_de_datos_pagerank.ipynb) de procesamiento, incluyendo las funciones para carga de teselas SRTM, obtención de coordenadas mediante Nominatim, consulta de rutas vía API de OSRM y cálculo de distancias 2D y 3D, se presenta en el Apéndice A de esta tesis.

### Validación metodológica

Se realizó una validación cruzada comparando las distancias 2D calculadas con mediciones oficiales del Instituto Nacional de Estadística y Geografía (INEGI) pueden consultar en el sitio oficial [Mapa Digital de México](https://gaia.inegi.org.mx/mdm6/) , obteniendo una discrepancia media inferior al $1.2\%$. Las distancias 3D no pudieron validarse directamente con fuentes externas (ningún servicio público proporciona esta métrica), pero se verificó la coherencia física mediante:
...
donde $d_{ij}^{(3D)}$ denota la distancia tridimensional calculada según la metodología descrita en la @sec-distancias3d. Esta formulación refleja la intuición de que municipios más cercanos ejercen una mayor influencia mutua dentro de la red, mientras que distancias mayores corresponden a una conectividad efectiva reducida.

### Matriz de adyacencia y normalización

La estructura del grafo se codifica mediante la [matriz de adyacencia](https://github.com/AngelaOrtiz25/tesis_angela/blob/gh-pages/datos%20y%20codigos/matriz_transicion_3d%20(2).csv) $N \in \mathbb{R}^{124 \times 124}$, cuyos elementos están dados por:
...
Esta modificación asegura que desde cualquier nodo colgante exista una probabilidad positiva de transitar hacia cualquier otro municipio de la red, preservando así la conservación de la probabilidad total en cada paso del proceso estocástico, tal como ocurre en los proceso estocásticos descritos en el [Capítulo 3](#sec-estocasticossegundo) 
...
Cabe precisar que el grado de salida de un nodo representa el número de aristas que emergen desde él hacia otros nodos, mientras que el grado de entrada corresponde al número de aristas que llegan a él desde otros nodos; en este contexto, el grado de salida cuantifica cuántas conexiones directas tiene un municipio hacia otros municipios a través de la red vial, y el grado de entrada indica cuántos municipios tienen rutas directas hacia él. El promedio se calcula sumando todos los grados (de salida o de entrada) y dividiendo entre el número total de municipios, lo que ofrece una medida de la densidad de conexiones típica del sistema, ya sea salientes o entrantes.

4. **Diámetro de la red**:La distancia máxima en el grafo, es decir, la longitud del camino más corto medida en número de aristas o "saltos" entre cualquier par de municipios es 4, lo cual indica una alta conectividad global del sistema territorial. Este indicador corresponde al concepto de **diámetro de la red** en teoría de grafos, el cual se define como la mayor de todas las distancias mínimas entre pares de nodos; un valor reducido como 4 sugiere que, independientemente del origen y destino, siempre es posible alcanzar cualquier municipio recorriendo a lo sumo cuatro conexiones intermedias a través de la infraestructura vial disponible.

### Matriz de transición modificada para PageRank {#sec-transicion2}

A pesar de las propiedades favorables mencionadas, la matriz $Q$ puede presentar convergencia hacia la distribución estacionaria (@sec-ergodicidad). Para garantizar la ergodicidad de la cadena de Markov y asegurar la existencia y unicidad de la distribución estacionaria, se introduce el factor de amortiguamiento $p \in (0,1)$, típicamente $p = 0.85$.
...
Estas propiedades garantizan, por el teorema de Perron-Frobenius (@sec-teroemafrobenius) para matrices estocásticas primitivas, la existencia de un único vector estacionario $\pi \in \mathbb{R}^{124}$ que satisface:
...
## Aplicación del algoritmo PageRank y convergencia {#sec-aplicaionpagerank}
...
Con estos ajustes, la matriz de transición mantiene la misma estructura ya mencionada en la @sec-transicion2.
...
El criterio de convergencia se basa en la norma $\ell_1$ [@meyer2000matrix; @kreyszig1989introductory] entre iteraciones consecutivas:
...
La aplicación del algoritmo PageRank a la red territorial de Chiapas produjo un vector estacionario $\pi$ cuyas componentes reflejan la importancia relativa de cada municipio. Los quince municipios con mayor valor PageRank se presentan en la @tbl-pagerank.
...
: Municipios con mayor centralidad según PageRank {#tbl-pagerank}

Del análisis de la @tbl-pagerank se desprenden tres hallazgos fundamentales.

**Primero**, once de los quince municipios con mayor centralidad pertenecen a la región de Los Altos de Chiapas. Este resultado se explica por la alta densidad municipal de dicha región, donde numerosos municipios de pequeña extensión territorial se encuentran separados por distancias cortas (10--40 km). Al ponderar las aristas con el inverso de la distancia tridimensional ($w_{ij} = 1/d_{ij}^{(3D)}$), estas conexiones generan pesos elevados que, acumulados a través de múltiples vecinos, producen una alta probabilidad estacionaria para los nodos de esta región.

**Segundo**, Tuxtla Gutiérrez, a pesar de ser la capital estatal y sede del almacén central de Protección Civil, se ubica en la posición 17 con un score de $\pi = 0.0100$. Este resultado, aparentemente contraintuitivo, valida la metodología al demostrar que el algoritmo captura la estructura real de conectividad vial sin sesgo por criterios administrativos o demográficos. La posición de Tuxtla se debe a su patrón de conectividad radial: sus conexiones hacia otras regiones del estado son predominantemente de larga distancia (40--120 km), lo que reduce los pesos individuales de sus aristas en comparación con los municipios de Los Altos, cuyas múltiples conexiones cortas generan mayor acumulación de probabilidad estacionaria.

**Tercero**, desde la perspectiva logística, los municipios con mayor centralidad identificada por PageRank funcionan como hubs naturales de redistribución regional. En un escenario de emergencia, una estrategia eficiente consistiría en enviar ayuda desde el almacén central de Tuxtla Gutiérrez hacia estos municipios prioritarios, los cuales, por su alta conectividad local, pueden redistribuir los recursos hacia comunidades adyacentes de manera rápida y con menor costo operativo.

El vector estacionario $\pi$ constituye así el insumo fundamental para la etapa siguiente, donde se determinarán las rutas óptimas desde Tuxtla Gutiérrez hacia cada uno de los municipios prioritarios identificados.

        
## Rutas óptimas desde Protección Civil hacia municipios prioritarios
La elección de Tuxtla Gutiérrez como punto de almacenamiento central a pesar de no ocupar los primeros lugares del ranking PageRank, Responde a criterios operativos y logísticos fundamentales que complementan el análisis de centralidad de la red.. En primer lugar, Tuxtla concentra la infraestructura necesaria para la recepción de ayuda externa: el Aeropuerto Internacional Ángel Albino Corzo, la sede estatal de Protección Civil y almacenes con capacidad operativa. En segundo lugar, la ayuda humanitaria en situaciones de emergencia proviene predominantemente de fuentes externas al estado (gobierno federal, organismos internacionales, donaciones nacionales), por lo que el punto de almacenamiento debe ubicarse en el nodo de entrada natural de estos recursos, no necesariamente en el nodo más central de la red interna. Finalmente, la metodología propuesta opera bajo una logística jerárquica de dos niveles: Tuxtla funciona como centro principal de recepción y despacho, mientras que los municipios con mayor PageRank (San Cristóbal, Chamula, Rayón, entre otros) funcionan como centros secundarios de redistribución regional. Esta separación de funciones resuelve la aparente contradicción y refleja la práctica real de los centros de coordinación de emergencias.

### Punto de almacenamiento y municipios estratégicos

El punto de almacenamiento central para operaciones de ayuda humanitaria se establece en las instalaciones de la [Secretaria de Protección Civil](https://maps.app.goo.gl/fRNWcaote5heu1Qp9) ubicadas en Tuxtla Gutiérrez, capital del estado de Chiapas. Esta localización responde a criterios operativos fundamentales:

1. **Accesibilidad desde el aeropuerto**: Conexión directa con el [Aeropuerto Internacional Ángel Albino Corzo](https://maps.app.goo.gl/kip9Nrw984oCGX7ZA) mediante vía terrestre de alta capacidad.
...
A partir de la aplicación del algoritmo PageRank descrita en la @sec-aplicaionpagerank, se identificaron los 15 municipios con mayor importancia relativa ($\pi_i$) dentro de la red territorial. Estos municipios constituyen los nodos estratégicos hacia los cuales se dirigen las rutas de distribución prioritaria en escenarios de emergencia.
...
La @tbl-rutas-optimas presenta las rutas óptimas calculadas desde Tuxtla Gutiérrez hacia los 15 municipios con mayor valor PageRank. Para cada destino, se detalla la secuencia de municipios que conforman el camino de menor costo, así como las métricas de distancia bidimensional (2D), distancia tridimensional (3D), el incremento porcentual debido al relieve y el tiempo estimado de recorrido.
...
| Rayon | Tuxtla Gutierrez → Copainala → Rayon | 126.5 | 128.3 | 1.5 | 162 |
| Chamula | Tuxtla Gutierrez → Chamula | 76.1 | 76.7 | 0.9 | 78 |
| Chiapilla | Tuxtla Gutierrez → Chiapilla | 67.8 | 68.1 | 0.4 | 76 |
| Pueblo Nuevo Solistahuacan | Tuxtla Gutierrez → Pueblo Nuevo Solistahuacan | 121.1 | 122.3 | 1.0 | 116 |
| Aldama | Tuxtla Gutierrez → Aldama | 94.2 | 95.3 | 1.1 | 110 |
| Rincon Chamula San Pedro | Tuxtla Gutierrez → Rincon Chamula San Pedro | 129.6 | 130.9 | 1.0 | 125 |
| Pantepec | Tuxtla Gutierrez → Copainala → Pantepec | 117.8 | 119.6 | 1.5 | 151 |
| Santiago el Pinar | Tuxtla Gutierrez → Ixtapa → Santiago el Pinar | 97.6 | 98.6 | 0.9 | 134 |
...
| Tapilula | Tuxtla Gutierrez → Copainala → Tapilula | 135.4 | 137.4 | 1.5 | 171 |
| Acala | Tuxtla Gutierrez → Acala | 52.8 | 52.9 | 0.2 | 60 |
| Mitontic | Tuxtla Gutierrez → Mitontic | 97.9 | 99.4 | 1.5 | 105 |

: Rutas óptimas desde Protección Civil (Tuxtla Gutiérrez) hacia los 15 municipios con mayor centralidad PageRank. {#tbl-rutas-optimas}

#### Análisis de los resultados de optimización

De la @tbl-rutas-optimas se desprenden observaciones operativas cruciales para la logística humanitaria en Chiapas:

1. **Impacto real de la topografía**: El incremento topográfico oscila entre el $0.2\%$ (Acala) y el $1.5\%$ (Rayon, Pantepec, Tapilula, Mitontic). Aunque estos porcentajes parecen modestos, representan kilómetros adicionales de recorrido real que impactan directamente en el consumo de combustible, el desgaste de frenos y la fatiga del conductor. Planificar con distancias 2D subestimaría sistemáticamente los costos operativos reales.

2. **Corredores de dependencia logística**: Se observa la formación de cuellos de botella estratégicos. El municipio de **Copainalá** actúa como nodo de transición obligatorio para alcanzar tres de los municipios prioritarios: Rayon, Pantepec y Tapilula. De manera similar, **Ixtapa** funciona como punto de paso hacia Santiago el Pinar. Estos nodos intermedios son puntos críticos de vulnerabilidad: cualquier interrupción en las carreteras que los atraviesan (por deslaves o daños) aislaría simultáneamente a múltiples destinos prioritarios.

3. **Estructura radial vs. conexiones directas**: Las rutas hacia municipios como Chamula, Aldama y Mitontic son conexiones directas desde Tuxtla Gutiérrez, lo que indica que el algoritmo identificó carreteras eficientes entre estos pares. En contraste, las rutas hacia la zona Norte requieren pasar por Copainalá, reflejando la estructura radial y jerárquica de la red vial en esa región montañosa.

4. **Realismo en la estimación del tiempo**: El tiempo estimado de recorrido varía significativamente, desde 56 minutos (San Cristobal) hasta 171 minutos (Tapilula). Es fundamental destacar que este tiempo no se calculó dividiendo la distancia por una velocidad promedio constante. En su lugar, es la **suma acumulativa de los tiempos por tramo** proporcionados por el motor OSRM, los cuales consideran variables reales como el tipo de superficie (pavimento/terracería), límites de velocidad legales y curvatura de la carretera. Esto ofrece una estimación de la duración del viaje mucho más fiable que los modelos tradicionales.

En conclusión, la combinación del algoritmo PageRank (para la selección estratégica de destinos basada en centralidad) y el algoritmo de Dijkstra con métrica 3D (para la planificación táctica de rutas) proporciona un modelo de distribución de ayuda humanitaria que es matemáticamente robusto, computacionalmente eficiente y geográficamente realista para las complejas condiciones del estado de Chiapas.
