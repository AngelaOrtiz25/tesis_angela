...
Elevación SRTM (30 m). Aplicando el algoritmo PageRank con factor de
...
Tapachula y San Cristóbal de las Casas. Las rutas óptimas se calcularon
mediante Dijkstra sobre métrica 3D, evidenciando incrementos de
distancia del 4.2\% en promedio (hasta 12\% en zonas serranas). En el
ámbito operativo, se construyó un sistema híbrido de \emph{nowcasting}
para variables críticas (viento, temperatura, humedad, precipitación) a
lo largo de corredores de vuelo, combinando estimación de velocidad de
advección mediante correlación cruzada y análisis frecuencia-espacio
...
directed graph using three-dimensional distances calculated via
Vincenty's formula and the 30 m resolution SRTM Digital Elevation Model.
By applying the PageRank algorithm with a damping factor (\(p=0.85\)), a
structural centrality vector is obtained that ranks municipalities
according to their capacity for logistics flow concentration; the
existence and uniqueness of this solution are guaranteed by the
Perron-Frobenius theorem. The algorithm identified strategic centers
such as Tuxtla Gutiérrez, Tapachula, and San Cristóbal de las Casas.
Optimal routes were computed using Dijkstra's algorithm over the 3D
metric, revealing average distance increments of 4.2\% (up to 12\% in
mountainous areas). In the operational domain, a hybrid
\emph{nowcasting} system was developed for critical variables (wind,
temperature, humidity, precipitation) along flight corridors, combining
advection velocity estimation via cross-correlation and
frequency-wavenumber (\(f\)-\(k\)) analysis, with temporal persistence
captured by AR(1) processes. Uncertainty was quantified using the
\emph{Moving Block Bootstrap} method, preserving serial dependence
without assuming normality. Finally, a binary decision rule under
uncertainty was formalized based on Wald's criterion and feasible
...
\begin{tcolorbox}[enhanced jigsaw, toptitle=1mm, bottomtitle=1mm, leftrule=.75mm, breakable, coltitle=black, colframe=quarto-callout-note-color-frame, title=\textcolor{quarto-callout-note-color}{\faInfo}\hspace{0.5em}{Nota}, colback=white, bottomrule=.15mm, colbacktitle=quarto-callout-note-color!10!white, arc=.35mm, titlerule=0mm, toprule=.15mm, rightrule=.15mm, left=2mm, opacityback=0, opacitybacktitle=0.6]
