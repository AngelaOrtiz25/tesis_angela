...
\begin{tcolorbox}[enhanced jigsaw, opacitybacktitle=0.6, breakable, bottomtitle=1mm, left=2mm, leftrule=.75mm, colbacktitle=quarto-callout-note-color!10!white, titlerule=0mm, colback=white, toprule=.15mm, arc=.35mm, title=\textcolor{quarto-callout-note-color}{\faInfo}\hspace{0.5em}{Nota}, rightrule=.15mm, toptitle=1mm, bottomrule=.15mm, opacityback=0, colframe=quarto-callout-note-color-frame, coltitle=black]
...
\subsection{Sensibilidad frente a condiciones meteorológicas
adversas}\label{sensibilidad-frente-a-condiciones-meteoroluxf3gicas-adversas}

La robustez del sistema integrado se evalúa mediante un análisis de
sensibilidad frente a la degradación de las condiciones meteorológicas.
Sea \(\theta \in [0,1]\) un parámetro de severidad meteorológica, donde
\(\theta = 0\) representa condiciones óptimas y \(\theta = 1\)
condiciones extremas. La probabilidad de vuelo operativo se modela como:

\[
p_{\text{vuelo}}(\theta) = p_0 \cdot (1 - \theta)^\kappa,
\]

con \(p_0 = 0.94\) (probabilidad base) y \(\kappa = 2.5\) (factor de
sensibilidad empírico).

La ilustra la relación entre severidad meteorológica y eficiencia
logística normalizada. Se observa que el sistema mantiene eficiencia
superior al 80\% para \(\theta < 0.4\), degradándose gradualmente para
condiciones más adversas. Esta característica es deseable, ya que
refleja una transición suave entre modos operativos en lugar de fallos
catastróficos.
