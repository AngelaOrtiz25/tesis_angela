...
discreto}\label{correlaciuxf3n-cruzada-y-existencia-del-lag-uxf3ptimo-discreto}
...
climática de baja frecuencia (\citeproc{ref-shumway2017time}{Shumway y
Stoffer 2017}).
...
es consistente: su varianza no converge a cero, incluso para \(T\)
grande. Para obtener estimadores consistentes se requiere suavizado
espectral, ya sea mediante ventanas espectrales (Daniell, Parzen,
Bartlett) o mediante promediado sobre frecuencias adyacentes
(\citeproc{ref-priestley1981spectral}{Priestley 1981}).

Alternativamente, los métodos paramétricos ajustan un modelo ARMA de
orden finito a la serie y derivan \(f(\omega)\) analíticamente a partir
de los coeficientes estimados. Estos enfoques son particularmente útiles
cuando la longitud muestral es limitada, como ocurre en ventanas de
nowcasting de 7 días con muestreo cada 10 minutos.
...
componentes centrales de esta tesis: 1. \textbf{Validación de
estacionariedad}: La inspección visual y cuantitativa de \(f(\omega)\)
permite detectar tendencias no estacionarias o ciclos dominantes que
deben eliminarse o modelarse explícitamente antes de aplicar métodos de
correlación cruzada. 2. \textbf{Análisis \(f\)-\(k\)}: La extensión de
la transformada de Fourier al dominio espacio-temporal \((s,t)\) permite
estimar la velocidad de fase de estructuras atmosféricas mediante la
pendiente de la fase espectral, metodología que se desarrolla en la
Sección~\ref{sec-analisisfk} y se aplica operativamente en la estimación
de velocidad de advección (Sección 4.3).
...
\begin{equation}\phantomsection\label{eq-espectropotencia}{
...
\item
  ...
  \] donde \(S\) esta definido en la ecuación
  (\citeproc{ref-espectropotencia}{\textbf{espectropotencia?}}) y la
  velocidad final se calcula como la mediana ponderada de
...
cruzada (Sección 2.3) en regímenes donde la coherencia temporal entre
segmentos adyacentes es baja pero existe estructura espectral clara. La
velocidad estimada \(v = \operatorname{median}_{\omega}\{v_p(\omega)\}\)
se incorpora directamente al operador de transporte advectivo discreto:
\[
s_i^{\text{up}} = s_i - v H,
\]

que constituye el núcleo del modelo híbrido de nowcasting desarrollado
en la Sección 4.4. Esta cadena teórica, desde la transformada 2D hasta
la regresión de fase, garantiza que el pronóstico respete la física de
propagación observada, transformando observaciones espacio-temporales en
parámetros operativos cuantificables y trazables.
...
la Sección 4.4, complementando el transporte advectivo con dinámica
temporal intrínseca y garantizando estabilidad numérica frente a
...
\] la cual converge en media cuadrática (\(L^2\)) dado que
\(\sum_{j=0}^\infty |\phi|^j = (1-|\phi|)^{-1} < \infty\)
(\citeproc{ref-brockwell2016time}{Brockwell y Davis 2016}).

A partir de esta representación se obtienen directamente los momentos
poblacionales: - \textbf{Media:} \(\mathbb{E}[X_t] = 0\) (para el
proceso centrado). - \textbf{Varianza:}
...
cercano al ruido blanco (\citeproc{ref-shumway2017time}{Shumway y
Stoffer 2017}).
...
En la implementación del sistema de nowcasting (Sección 4.4), esta
expresión se utiliza directamente para calcular el componente
autorregresivo: \[
...
es consistente y asintóticamente normal: \[
...
\(\hat{\phi}\) se infla cuando \(|\phi|\) se aproxima a \(1\) (raíz
...
particularmente crítica para: 1. \textbf{Temperatura del aire:} Donde la
inercia termodinámica justifica \(\phi \approx 0.95\), reflejando alta
coherencia temporal. 2. \textbf{Humedad relativa:} Con persistencia
moderada (\(\phi \approx 0.85\)), capturando la dinámica higroscópica
sin sobreajuste. 3. \textbf{Modelo de respaldo:} Cuando fallan los
métodos advectivos, el AR(1) regularizado actúa como fallback
conservador, evitando extrapolaciones físicamente inconsistentes.
...
\begin{tcolorbox}[enhanced jigsaw, title=\textcolor{quarto-callout-note-color}{\faInfo}\hspace{0.5em}{Nota}, rightrule=.15mm, bottomtitle=1mm, colframe=quarto-callout-note-color-frame, bottomrule=.15mm, titlerule=0mm, toprule=.15mm, opacitybacktitle=0.6, left=2mm, colbacktitle=quarto-callout-note-color!10!white, colback=white, toptitle=1mm, arc=.35mm, opacityback=0, leftrule=.75mm, breakable, coltitle=black]
